# Devine B2C WhatsApp Bot - Complete English Content Reference

This document contains all user-facing text strings, button labels, flow headers, and message templates used by the Devine B2C WhatsApp Chatbot.

---

## 1. Welcome & Language Selection Flow (First-Time Greeting)

### **Header Image**
- **Image**: Welcome Banner Image (16:9 / 1:1)

### **Welcome Message**
> Welcome to **Devine Natural & Organic Foods**! 🌿
>
> We deliver 100% pure, unadulterated cold-pressed oils, traditional wood-pressed spices, organic honey, and natural wellness products directly from pristine farms to your doorstep.
>
> Please choose your preferred language to continue:

### **Quick Reply Buttons**
1. `🇬🇧 English`
2. `🇮🇳 தமிழ்`

---

## 2. Language Selection Confirmation

### **English Selected Confirmation**
> ✅ **Language set to English!**
> How can we assist you today? Please tap below to explore our services.

---

## 3. Main Menu / Choose Service Flow

### **Service Options (Radio Selection / Buttons)**

1. **🛍️ Browse & Buy Products**
   - *Description*: View our complete range of cold-pressed oils, honey, & organic foods.
2. **📦 Track My Order**
   - *Description*: Check real-time delivery status of your active order.
3. **🎁 Corporate & Festive Gifting**
   - *Description*: Custom gift hampers for weddings, festivals, and corporate events.
4. **💬 Talk to Customer Support**
   - *Description*: Connect directly with our team on WhatsApp for help.
5. **🌐 Change Language / மொழி மாற்றம்**
   - *Description*: Switch between English and தமிழ் (Tamil).

---

## 4. Browse Products Flow

### **Catalog Category Selection**
> Select a category to explore fresh natural products:
>
> 1. 🛢️ Cold-Pressed Oils (Groundnut, Sesame, Coconut)
> 2. 🍯 Pure Organic Honey & Spices
> 3. 🌾 Traditional Grains & Millets
> 4. 🎁 Gift Packs & Combo Offers

---

## 5. Order Summary & WhatsApp Checkout Flow

### **Cart Details Header**
> 🛍️ **Your Devine Cart Summary**

### **Price Breakdown Template**
> - **Items Total**: ₹{subtotal}
> - **Delivery Charge**: {delivery_charge}
> - **Total Amount Payable**: ₹{total_amount}
>
> 📍 **Delivery Location**: {address}

### **Action Buttons**
- `💳 Pay Online (UPI / Card / NetBanking)`
- `💵 Cash on Delivery (COD)`
- `✏️ Edit Cart / Delivery Address`

---

## 6. Order Confirmation Message

> 🎉 **Order Placed Successfully!**
>
> **Order ID**: `{order_id}`
> **Tracking Code**: `{track_id}`
> **Payment Method**: `{payment_method}`
>
> Thank you for choosing Devine Natural Foods! You will receive live WhatsApp status updates when your parcel is dispatched.

---

## 7. Change Language Flow

### **Header**
> 🌐 **Select Language / மொழி தேர்ந்தெடுக்கவும்**

### **Options**
- **🇬🇧 English**
  - *Subtitle*: Continue in English
- **🇮🇳 தமிழ் (Tamil)**
  - *Subtitle*: தமிழில் தொடரவும்
